//
//  YogaScreenOverviewDetail.swift
//  MyFitnessMentor
//
...
//
class YogaScreenOverviewDetail: UIViewController, UIScrollViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        demonstrationImage.addGestureRecognizer(imagePress)
    ...
    func displayContent() {
        ...
        //
        demonstrationImage.image = demonstrationArray[sessionScreenIndex][0]
        demonstrationImage.animationImages = demonstrationArray[sessionScreenIndex]
        demonstrationImage.animationDuration = 3
        demonstrationImage.animationRepeatCount = 1
        ...
        // Title Labels
        // Breaths
        self.breathsLabel.text = (breathsArray[sessionScreenIndex] + " " + NSLocalizedString("breaths", comment: ""))
        // Progress
    ...
    @IBAction func nextButton(_ sender: Any) {
        ...
        if sessionScreenIndex == sessionArray.count - 1 {
            ...
            //
            let snapshot1 = self.view.resizableSnapshotView(from: CGRect(x: 0, y: 3, width: view.frame.size.width, height: view.frame.size.height - 3), afterScreenUpdates: false, withCapInsets: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0))
            snapshot1?.frame = CGRect(x: 0, y: 3, width: view.frame.size.width, height: view.frame.size.height - 3)
            //
            backButton.tintColor = colour4
            sessionScreenIndex = sessionScreenIndex + 1
            displayContent()
            //
            let snapshot2 = self.view.resizableSnapshotView(from: CGRect(x: 0, y: 3, width: view.frame.size.width, height: view.frame.size.height - 3), afterScreenUpdates: true, withCapInsets: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0))
            snapshot2?.frame = CGRect(x: 0, y: 3, width: view.frame.size.width, height: view.frame.size.height - 3)            //
    ...
    // Play Image Sequence
    @IBAction func imageSequenceAnimation(extraSwipe:UITapGestureRecognizer) {
        //
        if demonstrationArray[sessionScreenIndex].count != 1 {
            demonstrationImage.startAnimating()
        }
    }
    ...
    //
    //
    //            let centerY = breathsLabel.frame.maxY + navigationBarHeight + UIApplication.shared.statusBarFrame.height
