...
class YogaScreenOld: UIViewController, UIScrollViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func walkthroughMindBody() {
        ...
        case 13:
            ...
            let y = navigationBarHeight + UIApplication.shared.statusBarFrame.height + imageScroll.frame.size.height
            let yValue = y + breathsLabel.frame.size.height + bodyPartLabel.frame.size.height + timerButton.frame.size.height
            path.addRect(CGRect(x: 14 + progressLabel.frame.size.width, y: yValue, width: view.frame.size.width - 14 - progressLabel.frame.size.width, height: 24.5))
            path.addRect(screenSize)
            //
            let maskLayer = CAShapeLayer()
            maskLayer.backgroundColor = UIColor.black.cgColor
